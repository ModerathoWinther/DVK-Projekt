    ...
    def run(self, is_training=True):
        ...
        for episode in range(self.max_episodes):
            ...
            if is_training:
                ...
                log_message = f"[STATUS] |  {datetime.now().strftime(DATE_FORMAT)}  |  End of episode {episode}  |  n steps: {step_count} from row {env.current_step - step_count} in dataset\t\t|  win_rate: {win_rate:.2f}  |  Epsilon: {epsilon:.3f}  |  Sharpe: {episode_sharpe:.3f}\t|  (episode reward: {episode_reward:.5f})"
